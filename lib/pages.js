...
import { QUERY_ALL_PAGES, QUERY_HOME_PAGE, getQueryPageById, getQueryPageByUri, getQueryFrontPanelById } from 'data/pages';
...
export async function getFrontPanelById(id) {
  const apolloClient = getApolloClient();
  const data = await apolloClient.query({
    query: getQueryFrontPanelById(id),
  });
  const { frontPanelFields } = data.data.page;
  return {
    frontPanelFields
  };
}
